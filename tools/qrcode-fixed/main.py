...
class QRCode:
    ...
    def make_image(self, image_factory=None, version=0, qr_text='some text'):
        """
        ...
        im = image_factory(self.border, self.modules_count, self.box_size,
                           version, qr_text, **self.args)
        for r in range(self.modules_count):
            for c in range(self.modules_count):
                if self.modules[r][c]:
                    im.drawrect(r, c, *self.check_neighboors(r, c))
        return im

    def check_neighboors(self, row, column):
        try:
            left = bool(self.modules[row][column - 1])
        except IndexError:
            left = False
        try:
            right = bool(self.modules[row][column + 1])
        except IndexError:
            right = False
        try:
            up = bool(self.modules[row - 1][column])
        except IndexError:
            up = False
        try:
            bottom = bool(self.modules[row + 1][column])
        except IndexError:
            bottom = False

        return not (left | right | up | bottom), (left, up, bottom, right)
